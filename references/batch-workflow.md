...
本文件定义批处理外壳。单个模型仍使用 `audit_source.ps1` 和 `run_pipeline.ps1`；`run_batch.ps1` 负责把大量、来源各异的人物、动物、怪物、机械或车辆资源拆成独立任务，并保存可恢复状态。
...
相对贴图路径以配置文件所在目录为基准。`base_color_texture` 只用于已经从原始 Prefab/Material/DCC 关系中核验的单张基础色修复；复杂 PBR、透明或多材质模型仍必须建立明确通道映射。
...
每个 `separate` 任务在进入 `READY_FOR_STUDIO` 前自动把正式外部图片转换为 `*_Roblox.png`，移除应用元数据并完成结构/像素回读。`studio_import_plan.json` 只列标准化后的路径和哈希。默认最大边长 4096；需要移动端降到 2048/1024 时必须逐任务配置 `max_texture_dimension`，不能把一个全局质量决策静默套到不同模型。
...
Codex 负责审计、必要转换、图片处理、选项判断、准备队列和可用工具能够执行的 Studio 操作；`prepare_animation_import.ps1` 记录目标 Rig、体验身份和逐动作状态。新队列不预选休息姿势；检查后由 Codex 填入选项名称、理由与证据。用户只配合账号/权限、无法可靠控制的界面步骤，以及效果或性能预算选择。批处理到达 `READY_FOR_STUDIO` 不等于模型已能在游戏运行。

动画批量处理不能只靠一份路径列表宣称完成。公开的 Studio 动画导入调用会显示文件选择/导入界面；使用当前可验证的工具或 UI 循环。锁定目标 Rig，对该资源试播确认导入设置后，再逐文件导入并验证；不同骨架/导出条件拆开处理。默认先全部本地导入，用户预览后只发布选中的动作。

Studio 固定顺序：

1. 导入 `model_bind.fbx`；
2. 上传或复用贴图并验证直接依赖；
3. Codex 判断休息姿势等导入设置，先本地导入一个代表动作 `canary_animation`；
4. 验证时间推进、骨骼变化和视觉形变；
5. 再将剩余动作全部导入本地并试听，不默认发布；
6. 只发布选中的动作，按动作名登记 ID；检查准确 Universe 的权限，缺失才授权，再用新 Play 验证；
