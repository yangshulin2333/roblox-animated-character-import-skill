...
- 同一转换包的单动作 FBX 导入到已导入的 Custom Rig 时，导入设置固定选择休息姿势来源第二项“导入的骨架”；该结论不能套用到骨架层级或休息姿势不同的外部动作。
...
- 每个已发布 AnimationId 都必须单独授权给目标体验。协作者身份只提供编辑能力，不会自动保证个人名下动画能被另一所有者的体验在运行时加载。
